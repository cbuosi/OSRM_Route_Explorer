...
const CONFIG = {
    ...
    tileOsmAttribution: '&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a>',
    tileSatUrl: 'https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}',
...
let _idCounter = 0;
const nextId = () => 'p' + Date.now().toString(36) + (++_idCounter).toString(36);
...
const Utils = {
    ...
    isLat(v) { return v >= -90 && v <= 90; },
    isLng(v) { return v >= -180 && v <= 180; },
    isCoordPair(lat, lng) {
        const la = Number(lat), ln = Number(lng);
        return !Number.isNaN(la) && !Number.isNaN(ln) && this.isLat(la) && this.isLng(ln);
    },
    normalizeCep(cep) { return String(cep || '').replace(/\D/g, '').slice(0, 8); },
    ...
    maneuverIcon(type, modifier) {
        ...
        if (mod && mod.indexOf('left') >= 0 && byType === 'bi-arrow-return-right') return 'bi-arrow-return-left';
        if (mod && mod.indexOf('right') >= 0 && byType === 'bi-arrow-return-left') return 'bi-arrow-return-right';
        return byType;
    },
