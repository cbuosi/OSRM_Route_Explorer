...
(function () {
    ...
    function toast(message, type = 'info', opts = {}) {
        ...
        el.innerHTML = `
            <div class="d-flex">
                <div class="toast-body w-100">
                    ...
                    ${opts.progress ? `<div class="progress mt-1" style="height:4px">
                        <div class="progress-bar progress-bar-animate" style="width:0%"></div></div>` : ''}
                </div>
                <button type="button" class="btn-close btn-close-white me-2 m-auto" data-bs-dismiss="toast"></button>
            </div>`;
        container.appendChild(el);
        const bs = new bootstrap.Toast(el, { delay: opts.delay || 3500, autohide: !opts.progress });
        ...
        if (opts.progress) {
            ...
            const iv = setInterval(() => {
                ...
                if (p >= 100) { clearInterval(iv); bs.hide(); }
            }, opts.speed || 60);
        }
    }
    ...
    function showAlert(message, type = 'danger') {
        ...
        if (!cont) { toast(message, type, { delay: 5000 }); return; }
        clearAlert();
    ...
    function choiceModal(title, items, itemLabel) {
        return new Promise((resolve) => {
            ...
            items.forEach((it, i) => {
                ...
                b.addEventListener('click', () => { cleanup(); resolve(i); });
                list.appendChild(b);
            });
            const bs = new bootstrap.Modal(modalEl);
            function cleanup() { bs.hide(); modalEl.remove(); }
            modalEl.addEventListener('hidden.bs.modal', () => { cleanup(); resolve(-1); });
            bs.show();
        });
    }
    ...
    function renderInfo(info) {
        ...
        const items = [
            { icon: 'bi-sign-turn-right', l: 'Distância', v: U.formatDistance(info.distance) },
            { icon: 'bi-clock', l: 'Tempo', v: U.formatDuration(info.duration) },
            { icon: 'bi-speedometer2', l: 'Vel. média', v: info.avgSpeed.toFixed(1) + ' km/h' },
            ...
            { icon: 'bi-file-earmark-arrow-down', l: 'Tamanho JSON', v: info.bytesFormatted }
        ];
    ...
    function renderMatrix(matrix) {
        ...
        for (let i = 0; i < dims; i++) html += `<th class="text-end">P${i + 1}</th>`;
        html += '</tr></thead><tbody>';
        for (let i = 0; i < dims; i++) {
            ...
            for (let j = 0; j < dims; j++) {
                ...
                const val = v === null || v === undefined ? '—'
                    : (matrix.type === 'duration' ? (Math.round(v / 60) + ' min') : U.formatDistance(v));
    ...
    function renderHistory(entries) {
        ...
        if (!entries || entries.length === 0) {
            tbody.innerHTML = '<tr><td colspan="6" class="text-center text-muted py-3">Nenhuma consulta no histórico.</td></tr>';
            return;
        }
    ...
    window.Ui = {
        toast, showAlert, clearAlert, choiceModal,
        renderPoints, renderInfo, renderRoutes, renderSteps,
        renderMatrix, renderHistory, renderSuggestions,
        fillSettings, readSettings, renderEndpoint
    };
})();