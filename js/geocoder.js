...
const Geocoder = {
    ...
    async nominatim(query, params = {}) {
        const q = new URLSearchParams({ format: 'jsonv2', 'accept-language': 'pt-BR', ...params });
    ...
    async nominatimStructured({ street, city, state, postcode }) {
        const q = new URLSearchParams({
            format: 'jsonv2', 'accept-language': 'pt-BR', limit: 5, countrycodes: 'br'
        });
    ...
    async viacep(cep) {
        ...
        const data = await this.cached(url, () => this.fetchJSON(url), 24 * 60 * 60 * 1000);
    ...
    async resolveCEP(cep) {
        ...
        try {
            ...
            choices = r.filter(it => /brazil|brasil|br/i.test(it.label) && it.lat && it.lng);
