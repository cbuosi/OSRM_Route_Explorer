...
const OSRM = {
  ...
  coordStr(points) {
    return points
      .map((pt) => `${Number(pt.lng).toFixed(6)},${Number(pt.lat).toFixed(6)}`)
      .join(";");
  },
  ...
  async request(url) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(
          `HTTP ${status} ${res.statusText}${detail ? " — " + detail : ""}`,
        );
      }
      ...
      if (err.name === "TypeError")
        throw new Error("Servidor OSRM indisponível ou inacessível.");
      throw err;
    } finally {
      clearTimeout(timer);
    }
  ...
  async nearest(point, number = 1) {
    const url =
      this.base("nearest") +
      `${Number(point.lng).toFixed(6)},${Number(point.lat).toFixed(6)}` +
      "?" +
  ...
  async match(coords, { radiuses } = {}) {
    const url = this.buildRequest("match", coords, {
      ...
      geometries: "geojson",
      annotations: "true",
      radiuses: radiuses ? radiuses.join(";") : undefined,
    });
    return this.request(url);
  },
};

// Expõe o global para o app (usado como `O = window.Osrm`).
window.Osrm = OSRM;
