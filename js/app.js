...
const Ui = window.Ui, U = window.Utils, O = window.Osrm, Geo = window.Geocoder;
...
const map = new window.MapController($('#map'), {
    ...
    onMouseMove: (ll) => { if (state.coordsVisible) setCoords(ll.lat, ll.lng); },
...
function addPoint(latitude, longitude, source) {
    ...
    if (!U.isCoordPair(lat, lng)) { Ui.showAlert('Coordenadas inválidas. Verifique latitude/longitude.'); return false; }
    if (state.points.length >= settings.maxPoints) { Ui.toast('Limite de pontos atingido.', 'warning'); return false; }
...
function markers() {
    return state.points.map(p => ({ lat: p.lat, lng: p.lng, origin: p.source }));
}
...
// alias sem destaque de linha (usado após operações em lote)
function renderAll() {
    renderPointsWrap();
}
...
function editPoint(id) {
    ...
    if (lng === null) return;
    if (!U.isCoordPair(lat, lng)) { Ui.toast('Coordenadas inválidas.', 'warning'); return; }
    p.lat = Number(lat); p.lng = Number(lng);
    renderPointsWrap(id);
    scheduleCalc('route');
}
...
function present(res) {
    ...
    $('#jsonMeta').textContent = `tamanho: ${U.formatBytes(res.size)} · ${res.time.toFixed(0)} ms`;
    const urlEl = $('#urlView');
...
async function runEndpoint(service) {
    if (service === 'nearest') { runNearest(); return; }
    if (service === 'trip' && state.points.length < 3) { Ui.showAlert('O endpoint Trip precisa de pelo menos 3 pontos.'); return; }
    if (state.points.length < 2) { Ui.showAlert('São necessários pelo menos 2 pontos.'); return; }
    ...
    try {
        ...
        else if (service === 'table') res = await O.table(pts, { annotations: 'duration,distance' });
...
async function runNearest() {
    ...
    if (!p) { Ui.showAlert('Adicione ao menos um ponto de referência para o Nearest.'); return; }
    setLoading(true);
    try {
        ...
        if (w) {
            ...
            Ui.renderEndpoint('nearest', { snapped: { lat: w.location[1], lng: w.location[0] }, distance: U.formatDistance(w.distance || 0) });
        }
    } catch (err) {
...
function renderRouteResult(data) {
    ...
    if (!routes.length) { Ui.showAlert('Nenhuma rota retornada pelo OSRM.'); return; }
    state.routes = routes;
    state.selected = 0;
...
function renderInfo(i) {
    ...
    if (!r) { Ui.renderInfo(null); return; }
    const legs = r.legs || [];
    ...
    Ui.renderInfo({
        ...
        bbox: b ? `SW ${b[1].toFixed(5)}, ${b[0].toFixed(5)} → NE ${b[3].toFixed(5)}, ${b[2].toFixed(5)}` : null
    });
    const list = [];
    legs.forEach(l => (l.steps || []).forEach(s => list.push({
...
function straightDistance(order) {
    ...
    for (let k = 0; k < order.length - 1; k++) {
        const a = state.points[order[k]], b = state.points[order[k + 1]];
...
function renderTripResult(data) {
    ...
    map.clearRoutes();
    if (geo.length) {
        map.drawRoutes([{ geometry: { coordinates: geo } }], 0, settings.routeColor, settings.routeWidth);
...
function addHistory() {
    ...
    Store.addToHistory({
        when: U.nowStamp(),
        origin: state.points[0] ? `${state.points[0].lat.toFixed(5)}, ${state.points[0].lng.toFixed(5)}` : '—',
        ...
        waypoints: state.points.map(p => ({ lat: p.lat, lng: p.lng, source: p.source })),
        url: state.url,
        profile: settings.profile
    });
}

function refreshHistory() { Ui.renderHistory(Store.getHistory()); }

function loadHistory(i) {
    const h = Store.getHistory()[i];
    if (!h) return;
    state.points = (h.waypoints || []).map(w => ({ id: ++idSeq, lat: w.lat, lng: w.lng, source: w.source || 'map' }));
    state.url = h.url || '';
    renderAll();
    scheduleCalc('route');
...
function deleteHistory(i) { Store.removeFromHistory(i); refreshHistory(); }
function clearHistory() { Store.clearHistory(); refreshHistory(); }
...
function saveSettings() {
    ...
    map.map.setView([settings.defaultLat, settings.defaultLng], settings.defaultZoom);
}
...
function exportGeoJSON() {
    const r = state.routes[state.selected];
    if (!r) { Ui.showAlert('Calcule uma rota antes de exportar GeoJSON.'); return; }
    const fc = {
        type: 'FeatureCollection',
        features: [
            { type: 'Feature', properties: { name: 'route', distance_m: r.distance, duration_s: r.duration }, geometry: r.geometry },
    ...
    U.download('rota.geojson', JSON.stringify(fc, null, 2), 'application/geo+json');
}

function exportJSONraw() {
    if (!state.rawResponse) { Ui.showAlert('Sem resposta para exportar.'); return; }
    U.download('rota.json', JSON.stringify(state.rawResponse, null, 2), 'application/json');
}

function exportGPX() {
    const r = state.routes[state.selected];
    if (!r) { Ui.showAlert('Calcule uma rota antes de exportar GPX.'); return; }
    const pts = (r.geometry?.coordinates || [])
        .map(([lng, lat]) => `      <trkpt lat="${lat.toFixed(6)}" lon="${lng.toFixed(6)}"></trkpt>`).join('\n');
...
function bookmarkRoute() {
    if (!state.routes[state.selected]) { Ui.showAlert('Calcule uma rota antes de salvar.'); return; }
    addHistory();
...
const searchDebounced = U.debounce(async (q) => {
    if (q.trim().length < 3) { Ui.renderSuggestions([]); return; }
    let items;
    try { items = await Geo.searchAddress(q, 5); } catch (e) { items = []; }
...
function pickSuggestion(evt) {
    ...
    if (addPoint(s.lat, s.lng, 'endereco')) map.flyTo(s.lat, s.lng, Math.max(settings.defaultZoom, 16));
}

async function handleCEP() {
    const cep = $('#cepInput').value.trim();
    if (!cep) { Ui.showAlert('Informe um CEP.'); return; }
    setLoading(true);
    try {
        const choices = await Geo.resolveCEP(cep);
        if (!choices.length) { Ui.showAlert('Não foi possível encontrar o CEP informado.'); return; }
        if (choices.length === 1) { pickCep(choices[0]); return; }
        const i = await Ui.choiceModal(`Resultados para o CEP ${cep}`, choices, (c) => (c.label || '').slice(0, 90));
...
function pickCep(c) {
    if (addPoint(c.lat, c.lng, 'cep')) map.flyTo(c.lat, c.lng, Math.max(settings.defaultZoom, 16));
}
...
function hideCoords() { $('#mouseCoords').style.display = 'none'; }
...
function bindEvents() {
    ...
    document.querySelectorAll('input[name="endpoint"]').forEach(r => {
        r.addEventListener('change', e => { state.endpoint = e.target.value; clearAutoTimer(); });
    });
    ...
    $('#pointsTbody').addEventListener('click', (e) => {
        ...
        else if (act === 'up') { const p = state.points.find(q => q.id === id); if (p) movePoint(id, true); }
    ...
    $('#btnAddLatLng').addEventListener('click', () => {
        ...
        if (addPoint(lat, lng, 'latlng')) { $('#latInput').value = ''; $('#lngInput').value = ''; }
    });
    $('#btnClearPoints').addEventListener('click', () => { state.points = []; renderAll(); clearResults(); });
    ...
    $('#cepInput').addEventListener('keydown', e => { if (e.key === 'Enter') handleCEP(); });

    // endereço (autocomplete)
    $('#addrInput').addEventListener('input', e => searchDebounced(e.target.value));
    $('#addrSuggest').addEventListener('click', pickSuggestion);

    // ferramentas
    $('#btnCenter').addEventListener('click', () => {
        if (state.points.length >= 2) map.fitPoints();
        else map.flyTo(settings.defaultLat, settings.defaultLng, settings.defaultZoom);
    });
    ...
    $('#btnClearMap').addEventListener('click', () => { state.points = []; renderAll(); clearResults(); hideCoords(); });
    $('#chkScale').addEventListener('change', e => map.toggleScale(e.target.checked));
    $('#chkGrid').addEventListener('change', e => map.toggleGraticule(e.target.checked));
    $('#chkWpt').addEventListener('change', e => { state.wptVisible = e.target.checked; map.setPoints(markers(), state.wptVisible); });

    // cor / espessura
    $('#routeColor').addEventListener('input', e => { settings.routeColor = e.target.value; redrawActive(); });
    ...
    // resultados
    $('#btnCopyJson').addEventListener('click', () => copyText(state.rawResponse ? JSON.stringify(state.rawResponse, null, 2) : ''));
    ...
    $('#settingsModal').addEventListener('show.bs.modal', () => Ui.fillSettings(settings));
    ...
    // navbar
    document.querySelectorAll('[data-action="nova-rota"]').forEach(b => b.addEventListener('click', () => {
        state.points = [];
        renderAll();
        clearResults();
        hideCoords();
...
function copyText(t) {
    return U.copyText(t).then(ok => Ui.toast(ok ? 'Copiado!' : 'Falha ao copiar.', ok ? 'success' : 'danger'));
}
...
function init() {
    ...
    if (typeof window.L === 'undefined' || typeof window.MapController === 'undefined') {
        Ui.showAlert('A biblioteca de mapas (Leaflet) não pôde ser carregada. Verifique sua conexão ou o bloqueio das CDNs utilizadas.');
        console.error('[OSRM Route Explorer] Leaflet não carregou (window.L indefinido).');
        return;
    }
    try {
        map.init();
        ...
        Ui.showAlert('Falha ao inicializar o mapa: ' + (err && err.message ? err.message : err));
