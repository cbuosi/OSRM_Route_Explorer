...
const Graticule = L.GridLayer.extend({
  createTile(coords) {
    ...
    const step =
      GRID_SPACING[Math.min(Math.floor(coords.z), GRID_SPACING.length - 1)];
    const origin = this._map.project(nw, coords.z);
    ctx.strokeStyle = "rgba(13,110,253,.30)";
    ctx.lineWidth = 0.6;
    for (
      let lng = Math.floor(nw.lng / step) * step;
      lng <= se.lng;
      lng += step
    ) {
      const p = this._map.project(L.latLng(0, lng), coords.z);
      ctx.beginPath();
      ctx.moveTo(p.x - origin.x, 0);
      ctx.lineTo(p.x - origin.x, size.y);
      ctx.stroke();
    }
    for (
      let lat = Math.floor(se.lat / step) * step;
      lat <= nw.lat;
      lat += step
    ) {
      const p = this._map.project(L.latLng(lat, 0), coords.z);
      ctx.beginPath();
      ctx.moveTo(0, p.y - origin.y);
      ctx.lineTo(size.x, p.y - origin.y);
      ctx.stroke();
    }
    return tile;
  },
...
class MapController {
  ...
  init() {
      ...
      onAdd() {
        ...
        const mk = (label, key, active) => {
          const b = L.DomUtil.create(
            "button",
            "rac-tileset-item" + (active ? " active" : ""),
            box,
          );
          b.innerHTML = label;
          b.onclick = (e) => {
    ...
    this.scaleCtrl = L.control.scale({
      imperial: false,
      position: "bottomleft",
    });
    ...
    this.map.on("click", (e) => {
      ...
      if (t && t.classList && t.classList.contains("leaflet-marker-icon"))
        return;
      if (this.cb.onMapClick) this.cb.onMapClick(e.latlng);
    });
    this.map.on(
      "mousemove",
      (e) => this.cb.onMouseMove && this.cb.onMouseMove(e.latlng),
    );
    this.map.on(
      "mouseout",
      () => this.cb.onMouseLeave && this.cb.onMouseLeave(),
    );
  ...
  syncMarkers() {
    ...
    this._points.forEach((pt, i) => {
      const kind =
        i === 0 ? "start" : i === this._points.length - 1 ? "end" : "";
      ...
      m.on(
        "dragend",
        () =>
          this.cb.onWaypointMove && this.cb.onWaypointMove(i, m.getLatLng()),
      );
      m.bindPopup(
  ...
  fitBoundsCoords(coords) {
    ...
    if (!ll.length) return;
    this.map.flyToBounds(L.latLngBounds(ll), {
      padding: [30, 30],
      duration: 0.6,
    });
  }
  ...
  drawRoutes(routes, sel, color, weight) {
    ...
    this.routeLines = routes.map((r, i) => {
      const ll = (r.geometry?.coordinates || []).map(([lng, lat]) => [
        lat,
        lng,
      ]);
